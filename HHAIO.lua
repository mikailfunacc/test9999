...
if Champion == nil and myHero.charName == "Zeri" then
	...
	Champion = {
		...
		OnPostAttackTick = function(PostAttackTimer)
			Champion:PreTick()
			Champion.QTargets = Utils:GetEnemyHeroes(self.qRange) -- last change
	...
	function Champion:OnTick()
		...
		self.QTargets = Utils:GetEnemyHeroes(self.qRange)-- last change
		self:QLogic()
		self:RLogic()
...
if Champion == nil and myHero.charName == "Nilah" then
	...
	Menu.eks = Menu.e:MenuElement({id = "eks", name = "E Killsteal", value = true})
	...
	qRange = 600
	aaRange = myHero.range
	CastingR = myHero.activeSpell.name == "NilahR"
	...
	function Champion:ELogic()
		if not GG_Spell:IsReady(_E) then
			return
		end
		self:EKS()
	end
	...
	-- q combo
	function Champion:QCombo()
		if not self.IsCombo or self.IsHarass and Menu.q_combo:Value() then
			return
		end
		local target = self.AttackTarget ~= nil and self.AttackTarget
			or GG_Target:GetTarget(self.QTargets, DAMAGE_TYPE_PHYSICAL)
	...
	function Champion:EKS()
		if not GG_Spell:IsReady(_Q) and CastEQ then CastEQ = false end
		for i, target in ipairs(Utils:GetEnemyHeroes(700)) do     	
			
			if target and myHero.pos:DistanceTo(target.pos) <= 550 and Utils:IsValid(target) then
				local QDmg = getdmg("Q", target, myHero)
				local EDmg = getdmg("E", target, myHero)
				
				if CastEQ then
					if Utils:Cast(HK_E, target) and not CastingR then
						Utils:Cast(HK_Q)
					end	
				end	
				
				if Menu.eks:Value() and GG_Spell:IsReady(_Q) then
				...
				if Menu.eks:Value() then
					if myHero.pos:DistanceTo(target.pos) <= 550 and EDmg > target.health then
