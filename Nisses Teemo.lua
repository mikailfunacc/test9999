...
function Teemo:ComboUseQ()
    ...
    if ValidTarget(QTarget, QRange) and self:CanUse(_Q, "Combo") and self:SmoothChecks() then
...
function Teemo:HarassUseQ()
    local QTarget = GetTarget(QRange)
    if ValidTarget(QTarget, QRange) and self:CanUse(_Q, "Harass") and self:SmoothChecks() then
        Control.CastSpell(HK_Q, QTarget)
    end
end
