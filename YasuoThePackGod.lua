...
function Yasuo:LoadMenu()
    self.tyMenu = MenuElement({type = MENU, id = "14", name = "YasuoThePackGod"})
	self.tyMenu:MenuElement({name = " ", drop = {"Reworked by UberMidNight"}})
    ...
    self.tyMenu.combo:MenuElement({type = MENU, id = "Ult", name = "Ultimate"})		
        ...
        self.tyMenu.combo.Ult:MenuElement({name = " ", drop = {"BeyBlade need more than 1.43 AttackSpeed"}})
        self.tyMenu.combo.Ult:MenuElement({name = " ", drop = {"KeyBlade need more than 1.53 AttackSpeed"}})
        self.tyMenu:MenuElement({id = "activateKeyBladeCombo", name = "Activate KeyBlade Combo (G)", value = true})
        self.tyMenu:MenuElement({id = "activateBeyBladeCombo", name = "Activate BeyBlade Combo (T)", value = true})
        self.tyMenu:MenuElement({id = "activateAirBladeCombo", name = "Activate AirBlade Combo (Space)", value = true})	
		...
		self.tyMenu.combo.Ult:MenuElement({id = "Count", name = "Min Enemies for [R]", value = 1, min = 1, max = 5})
...
-- Move isKeyBladeComboActive outside of Yasuo:Combo()
local followCursor = false
local isKeyBladeComboActive = false
local HK_G = 71 -- Define HK_G as a global variable
            ...
            if Eobj and distance < myHero.pos:DistanceTo(target.pos) then
                if _G.SDK.Orbwalker:CanMove() and myHero.pos:DistanceTo(target.pos) > AArange then
                    ...
                    DelayAction(function()
                        ...
                        target = tmpTarget -- Make sure to update the target after casting E+Q
                        self:CastR(target)
...
function Yasuo:FullDmg(user)
	...
	local Damage = EDmg+QDmg+EDmg+QDmg+RDmg+ADmg+IDmg
...
function Yasuo:GetEDamge(obj)
    ...
    local AD = 0.3 * myHero.bonusDamage
    local AP = 0.9 * myHero.ap
